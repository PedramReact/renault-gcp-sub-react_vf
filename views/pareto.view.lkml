view: pareto {
  sql_table_name: `REACT_DEV_DATA.Pareto`
    ;;
  ...
  dimension: calcule_RVB_APV_VP {
    ...
    sql:
      ...
      case
                 ...
                 when ${pareto.type_protection} = "LUP" and ${cod_et_apv_prov} = 6 and ${datemaxapvprov_date} is not null

      then "C"

      when ${pareto.type_protection} = "GQU"
...
# if([type_protection]='GQU',"G"))
    ...
    sql: case when ${pareto.type_protection} = "GQU"

                         then 'C'

      when

      ${pareto.type_protection} = "LUP" and ${lbscodts} = "SERIE DEFINITIVE" and ${lbsstatusapp} = 5 and ${datephyreelap_date} IS NOT NULL

      then 'D'

      when

      ${pareto.type_protection} = "LUP" and ${lbscodts}= "SERIE DEFINITIVE" and ${lbsstatusapp} = 0 and ${dt_appli_prev_ser_def_date} IS NOT NULL and ${dt_appli_prev_ser_def_date} > CURRENT_DATE
...
########################################################################################################
  ...
  dimension: calcul_RVB_Immediate_VP {
    ...
         case
                  ...
                       when ${pareto.type_protection} = "LUP" and ${lbscodts} = "SERIE PROVISOIRE" and ${lbsstatusapp} = 5 and ${datephyreelap_date} is not null

      then "D"

      when ${pareto.type_protection} = "LUP" and ${lbscodts} = "SERIE PROVISOIRE" and ${lbsstatusapp} = 0 and ${dtappliprevsolprov_date} is not null and ${dtappliprevsolprov_date} < CURRENT_DATE
      ...
      when ${pareto.type_protection} = "LUP" and ${lbscodts} = "SERIE PROVISOIRE" and ${lbsstatusapp} = 9

      then "A"

      when ${pareto.type_protection} = "LUP" and ${lbscodts} = "SERIE DEFINITIVE " and ${lbsstatusapp} = 5 and ${datephyreelap_date} is not null

      then "A"

      when ${pareto.type_protection} = "GQU" and ${gq38_putinplace_date} is not null
...
########################################################################################################
  ...
  measure: Calcul_Date_Solution_APV_VP {
    ...
    sql: case
                  ...
                     when ${pareto.cod_et_apv_prov} = 6

      then ${pareto.datemaxapvprov_date}
...
###########################################################################################
  ...
  dimension: Site_Application_LUP_GQU  {
    type: string
    sql:
      if((${pareto.lbsite2} IS NOT NULL) ,${pareto.lbsite2},${pareto.gq05_plantdesc});;
  }
...
###########################################################################################
  ...
  dimension: EC_FM_Titre_LUP {
    ...
    type: string
    sql:
      if((${pareto.llbquestion} IS NOT NULL),
      SUBSTR( if(NOT STRPOS(${pareto.llbquestion}, "//") = 0,
      SUBSTR(${pareto.llbquestion} , STRPOS(${pareto.llbquestion}, "//")+2,
      length( ${pareto.llbquestion} ) - STRPOS(${pareto.llbquestion}, "//")+2), ""),1,
      STRPOS(if(NOT STRPOS(${pareto.llbquestion}, "//") = 0,
      SUBSTR(${pareto.llbquestion} , STRPOS(${pareto.llbquestion}, "//")+2,
      length( ${pareto.llbquestion} ) - STRPOS(${pareto.llbquestion}, "//")+2), ""), "//") +if(STRPOS(if(NOT STRPOS(${pareto.llbquestion}, "//") = 0,
      SUBSTR(${pareto.llbquestion} , STRPOS(${pareto.llbquestion}, "//")+2,
      length( ${pareto.llbquestion} ) - STRPOS(${pareto.llbquestion}, "//")+2), ""), "//") = 0 ,0 , -1
      )),"");;
  }
...
###########################################################################################
  ...
  dimension: Direction_Traitement_LUP_GQU {
    type: string
    sql:if(${pareto.type_protection} = "GQU" , ${factory_manufacturing_vehicle_label_fr},
      if(${pareto.type_protection} = "LUP" ,
        if(length(${pareto.lib_ident_pb})!=0 ,
          if(length(${pareto.lbspoletrait})!=0 ,
          Concat(${pareto.lbspoletrait},"/",${pareto.lib_ident_pb}),${pareto.lib_ident_pb}),${pareto.lbspoletrait}),null));;
  }
...
###########################################################################################
  ...
  dimension: Doublon {
    sql:
       case
            when
              (NOT CONTAINS_SUBSTR(${pareto.tags}   ,"Doublon")  OR  (${pareto.tags} IS NULL))
            then 'Yes'
...
###########################################################################################
  ...
  dimension: ig_test {
    ...
    type: number
    sql: case when (${pareto.type_protection} = ${pareto.Protection_garde} and ${pareto.protection} = ${Max_Test} ) OR (${pareto.type_protection} = ${pareto.Protection_garde} AND (${pareto.type_protection} = "GQU")) then ${id_ig} end  ;;
  }
...
##EXPLORE: if( NOT is_null(max(${pareto.efficiency_rate})) ,max(${pareto.efficiency_rate}), null )
  ...
  measure: Taux_Eff{
    type: number
    sql: if((max(${pareto.efficiency_rate}) IS NOT NULL) ,max(${pareto.efficiency_rate}), null )  ;;
  }
}
